...
Destroys all `Pollen.Type` that exist and clears all their data.

| Parameter | Datatype | Purpose |
|------------|-----------|----------|
| None |  |  |

### `TypeReset(type)` → `undefined`
Resets all the underlying data of a part type to default settings similar to GM's 'part_type_clear' function. **NOTE: this does not clear the visual representation of particles!**

| Parameter | Datatype | Purpose |
|------------|-----------|----------|
| `type` | Pollen.Type | The `Pollen.Type` data that you want to reset |